...
人口動態、実質GDP、物価、および交易条件を外生条件としたシミュレーションの結果、実質GDPの持続的成長を仮定した場合であっても、将来の成長パターンが「資本集約的」である（すなわち労働需要のGDP弾力性が低い）ケースにおいては、供給側の労働力率上昇と相まって、人口減少下であっても労働市場がタイト化せず、かえって失業率が上昇基調をたどるという「マクロ需給のねじれ」のシナリオが示された。この挙動は、単なるマクロ成長の追求だけでなく、雇用の吸収力（労働集約度）やミスマッチ緩和策の成否が将来の雇用安定に影響を与えることを示唆している。
...
| `D_GDP` | GDPデフレーター（外生） | 内閣府「四半期別GDP統計」、 内閣府「中長期の経済財政に関する試算（過去投影ケース） |
...
| `TT` | 交易条件（外生） | 日本銀行「企業物価指数」（輸出物価指数・輸入物価指数、2026年以降は横置き仮定） |
...
combined <- combined |>
  dplyr::mutate(
    E_RY = dplyr::if_else(!is.na(E) & !is.na(RY) & RY != 0, E / RY, NA_real_)
  )
...
上記の主要指標に加えて、経済成長がどの程度の雇用吸収力を伴っているかを確認するため、労働集約度を `E / RY`（実質GDP1単位当たりの就業者数）として算出する。この指標は、実質GDPの増加に対して就業者数がどの程度必要とされるかを示す誘導指標であり、低下傾向は、同じ実質GDPをより少ない労働投入で生産する方向、すなわち資本集約化・労働節約的な成長パターンが強まっている可能性を示す。

```{r labor-intensity-plot, fig.width=10, fig.height=5}
labor_intensity_df <- combined |>
  dplyr::filter(data_type %in% c("actual", "simulation")) |>
  dplyr::filter(!is.na(E_RY))

ggplot(labor_intensity_df, aes(x = year, y = E_RY, color = data_type)) +
  geom_line(linewidth = 1) +
  geom_point(size = 1.5) +
  geom_vline(xintercept = max(hist_years) + 0.5, linetype = "dashed", color = "gray50") +
  scale_color_manual(
    values = c(actual = "#1f77b4", simulation = "#d62728"),
    labels = c(actual = "実績", simulation = "予測")
  ) +
  labs(
    title = "労働集約度（E / RY）の推移",
    x = "Year",
    y = "E / RY",
    color = NULL
  ) +
...
労働集約度の推移をみると、予測期間においても `E / RY` が低下する場合、実質GDPの増加ほどには就業者数が増えないことを意味する。このため、人口減少によって労働供給が制約される局面であっても、需要側の雇用吸収力が弱まれば、労働市場のタイト化ではなく、失業率の上昇や需給ミスマッチの拡大として表れる可能性がある。ただし、本指標は `E` と `RY` から計算されるモデル内の派生指標であり、産業別の資本深化や技術代替を直接識別するものではない。
...
+ 期待形成の明示化とミクロ的基礎づけ: フォワードルッキングな期待（インフレ期待等）を導入し、Lucas批判をクリアする動学的最適化モデル（DSGE等）へのアプローチを試みる。

+ 労働市場の摩擦のモデル化: サーチ・マッチング（Search and Matching）理論（Mortensen-Pissarides型）を取り入れ、UV曲線（ベバリッジ曲線）のシフトとして摩擦的失業を精緻に捉える。

+ 資本コストと代替弾力性の推計: 生産関数アプローチを明示化し、CES（Cobb-Douglas型に限定しない）生産関数等により、資本・労働の代替の弾力性を直接扱う。

+ 複数外生シナリオ（感度分析）の拡充: 内閣府の成長実現ケース／ベースラインケースに準拠した複数成長シナリオを構築。また、予測値のバンド（信頼区間）を提示する。
...
## （注記）
レポジトリに別途掲載しているLabourForce.rでは、労働力率の断層を調整するアドファクター（残差lfpr_last_obs_residualを10年で解消）を追加しており、労働力人口および就業者数の推移は、より自然な形になります。
