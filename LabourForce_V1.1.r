...
if (!"D_C" %in% names(data)) {
  stop("data/data_LabourForce.csv must contain D_C for real consumption.")
}
...
data_obs <- tibble(year = all_years) |>
  ...
  dplyr::mutate(
    ...
    LS       = LS * 10,                     # 労働力人口の実績値
    E        = E * 10,                      # 就業者数の実績値
    MHD      = E * H,                       # Labor demand in man-hours
    MHS      = LS * H,                      # Labor supply in man-hours
    U        = LS - E,                      # 失業者数の実績値
    E_RY     = E / RY,                      # 実質GDPあたり就業者数（労働需要の投入係数）
    MHD_RY   = MHD / RY,                    # Labor demand man-hours per real GDP
  ...
  dplyr::select(
    date, POP, RY, D_GDP, D_C, C_REAL_PC, P, R_INT, LS, PartRate, lgtPartRate,
    H, lgtH, MHD, MHS, E, E_RY, MHD_RY, U, U_rate, lgtU_rate, E_est, W, TT
  ) |>
...
estimation_check_data <- data_obs |>
  dplyr::mutate(
    ...
    MHD_MHS = MHD / MHS,
    MHD_MHS_lag1 = dplyr::lag(MHD / MHS, 1),
    lgtU_rate_lag1 = dplyr::lag(lgtU_rate, 1),
    lgtU_rate_lag2 = dplyr::lag(lgtU_rate, 2),
    log_E = log(E),
    log_H = log(H),
    log_H_lag1 = dplyr::lag(log(H), 1),
    log_H_lag2 = dplyr::lag(log(H), 2),
    lgtH_lag1 = dplyr::lag(lgtH, 1),
    lgtH_lag2 = dplyr::lag(lgtH, 2),
    log_MHD = log(MHD),
    log_MHD_lag1 = dplyr::lag(log(MHD), 1),
    log_MHD_lag2 = dplyr::lag(log(MHD), 2),
    ...
    log_W = log(W),
    log_W_lag1 = dplyr::lag(log(W), 1),
    log_W_lag2 = dplyr::lag(log(W), 2),
    log_C_REAL_PC = log(C_REAL_PC),
    log_C_REAL_PC_lag1 = dplyr::lag(log(C_REAL_PC), 1),
...
demand_specs <- list(
  manhour_level = stats::lm(
    log(MHD) ~ log_RY + log_W_D_GDP_lag1 + log_MHD_lag1,
...
demand_spec_diagnostics <- lapply(names(demand_specs), function(spec_name) {
  ...
  data.frame(
    spec = spec_name,
    real_hourly_wage_coef = wage_row[1, "Estimate"],
    real_hourly_wage_t = wage_row[1, "t value"],
...
hours_supply_specs <- list(
  bounded_dynamic_household = stats::lm(
    lgtH ~ log_W_P + log_C_REAL_PC + lgtH_lag1,
    data = estimation_check_data
  )
)

hours_supply_diagnostics <- lapply(names(hours_supply_specs), function(spec_name) {
  fit <- hours_supply_specs[[spec_name]]
  fit_summary <- summary(fit)
  coef_table <- stats::coef(fit_summary)
  wage_row <- coef_table[grep("log_W_P", rownames(coef_table), fixed = TRUE), , drop = FALSE]
  cons_row <- coef_table[grep("log_C_REAL_PC", rownames(coef_table), fixed = TRUE), , drop = FALSE]
  data.frame(
    spec = spec_name,
    real_hourly_wage_coef = wage_row[1, "Estimate"],
    real_hourly_wage_t = wage_row[1, "t value"],
    real_consumption_coef = cons_row[1, "Estimate"],
    real_consumption_t = cons_row[1, "t value"],
    adj_r_squared = fit_summary$adj.r.squared,
    aic = stats::AIC(fit),
    dw_stat = sum(diff(stats::residuals(fit))^2) / sum(stats::residuals(fit)^2),
    stringsAsFactors = FALSE
...
print(hours_supply_diagnostics)
...
iv_specs <- list(
  ...
  MHD = list(
    IV = c("1", "LOG(RY)", "TSLAG(LOG(W / D_GDP), 2)", "TSLAG(LOG(MHD), 2)"),
    y_var = "log_MHD",
    structural_vars = c("const", "log_RY", "log_W_D_GDP_lag1", "log_MHD_lag1"),
    endog_vars = c("log_W_D_GDP_lag1", "log_MHD_lag1"),
    reduced_vars = c("const", "log_RY"),
    x_vars = c("log_W_D_GDP_lag1", "log_MHD_lag1"),
    z_vars = c("const", "log_RY", "log_W_D_GDP_lag2", "log_MHD_lag2")
  ),
  lgtH = list(
  ...
  lgtU_rate = list(
    IV = c("1", "TSLAG(MHD / MHS, 1)", "TSLAG(lgtU_rate, 2)"),
    y_var = "lgtU_rate",
    structural_vars = c("const", "MHD_MHS", "lgtU_rate_lag1"),
    endog_vars = c("MHD_MHS", "lgtU_rate_lag1"),
    reduced_vars = "const",
    x_vars = c("MHD_MHS", "lgtU_rate_lag1"),
    z_vars = c("const", "MHD_MHS_lag1", "lgtU_rate_lag2")
  ),
  W = list(
...
BEHAVIORAL> lgtH TSRANGE 1996 1 2025 1
EQ> lgtH = h1 + h2 * LOG(W / P) + h3 * LOG(C_REAL_PC) + h4 * TSLAG(lgtH, 1)
COEFF> h1 h2 h3 h4

COMMENT> Labour Demand Side: Man-hours
BEHAVIORAL> MHD TSRANGE 1996 1 2025 1
EQ> LOG(MHD) = c1 + c2 * LOG(RY) + c3 * TSLAG(LOG(W / D_GDP), 1) + c4 * TSLAG(LOG(MHD), 1)
COEFF> c1 c2 c3 c4
...
EQ> lgtU_rate = a1 + a2 * (MHD / MHS) + a3 * TSLAG(lgtU_rate, 1)
COEFF> a1 a2 a3
...
ur_eq_map <- list(
  lgtH        = hours_supply_specs[["bounded_dynamic_household"]],
  MHD         = demand_specs[["manhour_level"]],
  lgtPartRate = supply_specs[["current"]],   # 労働供給：労働参加率（ロジット）
  lgtU_rate   = stats::lm(                 # 失業率（ロジット）：IV診断用 OLS 近似
    lgtU_rate ~ MHD_MHS + lgtU_rate_lag1,
    data = estimation_check_data
  ),
...
TABIT(model$simulation$H) # 労働時間の予測値
TABIT(model$simulation$LS) # 労働力人口の予測値
TABIT(model$simulation$MHS) # 労働供給マンアワーの予測値
TABIT(model$simulation$E) # 就業者数の予測値
TABIT(model$simulation$MHD) # 労働需要マンアワーの予測値
...
# ---------------------------------------------------------
simulation_vars <- c("POP", "RY", "D_GDP", "D_C", "P", "PartRate", "ADD_LFPR", "U_rate",
                     "W", "H", "LS", "MHS", "E", "MHD", "E_RY", "MHD_RY", "E_est", "U")
...
actual_df <- data_obs |>
  ...
  dplyr::select(year, POP, RY, W, H, PartRate, U_rate, LS, MHS, E, MHD, E_est) |>
  dplyr::mutate(
    employment_rate = E_est / POP,
    manhour_per_pop = MHD / POP,
    data_type = "actual"
  )
...
sim_df <- sim_df |>
  dplyr::mutate(
    employment_rate = ifelse(!is.na(E_est) & !is.na(POP), E_est / POP, NA_real_),
    manhour_per_pop = ifelse(!is.na(MHD) & !is.na(POP), MHD / POP, NA_real_)
  )
...
exog_df <- data_obs |>
  dplyr::mutate(year = lubridate::year(date)) |>
  dplyr::filter(year %in% proj_years) |>
  dplyr::select(year, POP, RY, W = W, H = H, PartRate = PartRate, U_rate = U_rate, LS = LS, MHS = MHS, E = E, MHD = MHD, E_est = E_est) |>
  dplyr::mutate(
    data_type = "exogenous"
  )
...
p5 <- ggplot(le_df, aes(x = year, y = value, color = series, linetype = data_type)) +
  geom_line() + geom_point(size = 1) +
  labs(title = "労働力人口 (LS) と 実現就業者数 (E_est)", x = "Year", y = "Number of People (thousands)") +
  theme_minimal()
...
p6 <- ggplot(mh_df, aes(x = year, y = value, color = series, linetype = data_type)) +
  geom_line() + geom_point(size = 1) +
  labs(title = "労働供給マンアワー (MHS) と 労働需要マンアワー (MHD)", x = "Year", y = "Man-hours") +
  theme_minimal()
